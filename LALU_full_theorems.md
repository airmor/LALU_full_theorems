...
**定义 20.5 (共轭转置)**  对 $A=(a_{ij})_{m\times n}$，$A^H=(\overline{a_{ji}})_{n\times m}$。

**定理 20.10 (伴随的矩阵表示)**  若 $T$ 在标准正交基下矩阵为 $A$，则 $T^*$ 的矩阵为 $A^H$。

**定理 20.11 (伴随的核与像)**  $\ker T^*=(\operatorname{im}T)^\perp$，$\operatorname{im}T^*=(\ker T)^\perp$，$\ker T=(\operatorname{im}T^*)^\perp$，$\operatorname{im}T=(\ker T^*)^\perp$。

**定理 20.12 (伴随与特征值)**  $\lambda$ 是 $T$ 的特征值 $\iff\overline{\lambda}$ 是 $T^*$ 的特征值。

**定理 20.13 (伴随与不变子空间)**  $U$ 在 $T$ 下不变 $\iff U^\perp$ 在 $T^*$ 下不变。

**定理 20.14 (酉/正交矩阵的等价刻画)**  以下等价：$S$ 是酉（正交）变换；$S$ 可逆且 $S^{-1}=S^*$；$S$ 将某组标准正交基映为标准正交基。酉/正交矩阵的特征值模为 $1$；实正交变换可正交相似于 $\pm1$ 块与 $2\times2$ 旋转块的分块对角。

**定理 20.15 (实谱定理)**  实内积空间上 $T$ 自伴 $\iff V$ 有由 $T$ 的特征向量构成的标准正交基 $\iff T$ 可正交对角化。自伴算子的特征值均为实数。

**定理 20.16 (正规算子的等价条件)**  $T$ 正规 $\iff\|Tv\|=\|T^*v\|$ 对所有 $v$；$T$ 正规时，不同特征值的特征向量正交；$T$ 正规时，$U$ 不变则 $U^\perp$ 也不变。

**定理 20.17 (正规算子的分解与交换性)**  任意 $T\in\mathcal{L}(V)$ 可分解为 $T=T_1+iT_2$，其中 $T_1=(T+T^*)/2$，$T_2=(T-T^*)/(2i)$，且 $T$ 正规 $\iff T_1T_2=T_2T_1$。

**定理 20.18 (自伴算子的进一步刻画)**  $T$ 自伴 $\iff\langle Tv,v\rangle\in\mathbf{R}$ 对所有 $v$；若 $T$ 自伴且 $\langle Tv,v\rangle=0$ 对所有 $v$，则 $T=0$。

**定理 20.19 (正算子的完整刻画)**  $T$ 正 $\iff T$ 自伴且特征值 $\ge0\iff T$ 有正平方根 $\iff T$ 有自伴平方根 $\iff\exists R$ 使 $T=R^*R$。

**定理 20.20 (镜像变换)**  对单位向量 $e$，$\varphi(x)=x-2\langle e,x\rangle e$ 是正交变换且 $\det\varphi=-1$。

**定理 20.21 (交换的正规算子可同时酉对角化)**  若 $T_1,T_2$ 均为正规算子且可交换，则它们可同时酉对角化。
