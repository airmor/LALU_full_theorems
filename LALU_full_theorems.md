...
说明：本文件按原讲义章节顺序汇总所有定义、定理与引理；无标签项将以简短描述标识并编号。数学公式使用 KaTeX 语法。已对照 [LALU定理汇总3.md](LALU定理汇总3.md) 做过人工复核，并把其中原先压缩或遗漏的关键结论补入文末专题。
...
#### 附录A 对照 LALU定理汇总3 的补充专题

##### A.1 奇异值分解与极分解

**定理 A.1 (T^*T 的基本性质)**  设 $T\,\in\,\mathcal{L}(V,W)$，则
1. $T^*T$ 是正算子；
2. $\ker(T^*T)=\ker T$；
3. $\operatorname{im}(T^*T)=\operatorname{im}T^*$；
4. $\dim\operatorname{im}T=\dim\operatorname{im}T^*=\dim\operatorname{im}(T^*T)$。

**定义 A.1 (奇异值与奇异向量)**  设 $T\in\mathcal{L}(V,W)$，若 $\lambda$ 是 $T^*T$ 的特征值，则 $\sqrt{\lambda}$ 称为 $T$ 的一个奇异值。若 $Tv=sw$ 且 $T^*w=sv$，则称 $v$、$w$ 分别为关于奇异值 $s$ 的左、右奇异向量。

**定理 A.2 (奇异值分解)**  若 $T$ 的正奇异值为 $s_1,\dots,s_m$，则存在 $V$ 的标准正交组 $e_1,\dots,e_m$ 与 $W$ 的标准正交组 $f_1,\dots,f_m$，使得对任意 $v\in V$，
$$
Tv=s_1\langle v,e_1\rangle f_1+\cdots+s_m\langle v,e_m\rangle f_m.
$$

**定理 A.3 (矩阵形式的奇异值分解)**  对任意矩阵 $A$，存在酉矩阵 $U,V$ 与非负实矩形对角矩阵 $\Sigma$，使
$$
A=U\Sigma V^*.
$$

**定理 A.4 (极分解)**  任意线性算子 $T$ 都可写成 $T=SP$，其中 $P=(T^*T)^{1/2}$ 为正算子，$S$ 为等距同构；在可逆情形下 $S$ 为酉（或正交）算子。

**推论 A.1 (奇异值与单满射)**  $T$ 单射当且仅当 $0$ 不是奇异值；正奇异值的个数等于 $\dim\operatorname{im}T$；在有限维情形，$T$ 满射当且仅当正奇异值个数等于 $\dim W$。

##### A.2 二次型、合同与正定性

**定理 A.5 (二次型矩阵表示唯一性)**  若对任意 $X$ 有 $X^\mathsf{T}AX=X^\mathsf{T}BX$，则 $A=B$。

**定理 A.6 (反对称矩阵刻画)**  $A$ 为反对称矩阵当且仅当对任意 $X$ 都有 $X^\mathsf{T}AX=0$。

**定理 A.7 (对称矩阵合同对角化)**  任意对称矩阵都可通过合同变换化为对角矩阵。

**定理 A.8 (惯性定理)**  实对称矩阵的合同规范形唯一，正、负、零惯性指数在合同变换下不变。

**定理 A.9 (正定性判据)**  实对称矩阵正定当且仅当其所有顺序主子式都大于 $0$；等价地，其所有特征值都大于 $0$。

**定理 A.10 (Cholesky 分解)**  正定矩阵 $A$ 可唯一写成 $A=LL^\mathsf{T}$，其中 $L$ 为对角元全正的下三角矩阵。

**定理 A.11 (半正定矩阵刻画)**  实对称矩阵半正定当且仅当其特征值全非负；等价地，可写成 $A=C^\mathsf{T}C$。

**定理 A.12 (正定矩阵与内积)**  若 $A$ 为实对称正定矩阵，则 $\langle x,y\rangle_A=x^\mathsf{T}Ay$ 定义了一个内积；反之，任意内积在标准坐标下都对应一个半正定 Gram 矩阵。

##### A.3 线性空间与商空间的补充结论

**定理 A.13 (子空间维数单调性)**  若 $U\subseteq W\subseteq V$，则 $\dim U\le\dim W$；若 $\dim U=\dim W$，则 $U=W$。

**定理 A.14 (向量组等价与秩)**  向量组与其极大线性无关组等价；任意两个极大线性无关组长度相等，因此秩唯一。

**定理 A.15 (仿射子集判别)**  非空子集 $A$ 是仿射子集，当且仅当对任意 $v,w\in A$ 与 $\lambda\in\mathbf{F}$，有 $\lambda v+(1-\lambda)w\in A$。

**定理 A.16 (商空间维数公式)**  若 $U$ 是有限维空间 $V$ 的子空间，则 $\dim(V/U)=\dim V-\dim U$。

**方法 A.1 (覆盖定理的使用方式)**  证明有限个非平凡子空间的并不能覆盖整个空间时，常先证 $s=2$，再用归纳法推广到一般 $s$。

##### A.4 参考性的几何补充

**定理 A.17 (三维空间的几何量)**  在三维欧氏空间中，$\|u\times v\|$ 表示平行四边形面积，$|[u,v,w]|$ 表示平行六面体体积。

**定理 A.18 (二次曲面的标准化框架)**  任意二次曲面可经正交变换与平移化为主轴标准形；其类型由二次型的符号结构与常数项共同决定。

**注 A.1**  以上补充项按“先结构、后应用”的顺序整理，主要用于弥补总表中原先被压缩掉的结论层级；与正文各章内容互相独立，但可直接作为复习索引使用。
